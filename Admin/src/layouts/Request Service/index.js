...
function RequestServicesView() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`${apiBase}/requestservices`);
        ...
        if (mounted) {
          if (Array.isArray(data)) {
            ...
            console.warn("/requestservices returned non-array:", data);
          }
        }
      } catch (err) {
        // eslint-disable-next-line no-console
        console.error("Error fetching request services:", err);
        if (mounted) setError("Failed to fetch request services.");
      } finally {
  ...
  const columns = [
    ...
    { Header: "Service Type", accessor: "Type", width: "25%", align: "left" },
  ...
  const rows = requests.map((item) => ({
    ...
    Type: <SubText text={item.requestservicestype || "—"} />,
    Description: <SubText text={item.description || "—"} />,
  }));
              ...
              <MDBox pt={3}>
                {requests.length === 0 ? (
                  <MDBox p={4} textAlign="center">
                    <MDTypography variant="h6">No requests found.</MDTypography>
                    <MDTypography variant="caption" color="text">
                      Click &quot;Add New&quot; to create the first service request.
                    </MDTypography>
                  </MDBox>
                ) : (
                  <DataTable
...
export default RequestServicesView;
